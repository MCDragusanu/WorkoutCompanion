package com.example.workoutcompanion.on_board.registration.view_model
...
class OnBoardViewModel @Inject constructor(
            ...
            if (!isOnline) onError(AuthManager.Errors.NoInternetConnection)
                ...
                if (isValid) authManager.registerUser(
                    ...
                    onSuccess = { uid ->
                        ...
                        saveUserInfo(uid)
                        cacheExerciseDatabase(onComplete = { viewModelScope.launch { this@OnBoardViewModel.navigateToNextScreen?.let { it1 -> it1(it) } } })
    ...
    private fun saveUserInfo(uid:String) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
           ...
           val result =  userRepo.addProfile(userUid = profile.uid , newProfile = profile)
            if(result.isFailure){
