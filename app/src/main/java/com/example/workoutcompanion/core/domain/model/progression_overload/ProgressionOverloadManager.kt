...
class ProgressionOverloadManager {
    ...
    fun convertStartingConditionToPlan(
        setCount : Int ,
    ...
    fun generateProgression(
        ...
        //I have to adapt the starting condition to match the rest of the program to avoid stupid weight jumps
        //ex if the user has entered the one rep max
        //the algo will have the first solution with 1 rep and 1RM
        //and then will start form example 10 reps with 1RM
        val result =
            mutableListOf(convertStartingConditionToPlan(4 , initialWeek , progressionSchema))

        for (weekIndex in 0 until weekCount) {

            val solution = generateSolution(
                previousWeek = result.last() ,
    ...
    fun findStartingPoint(
        ...
        return convertStartingConditionToPlan(
            ...
            Week(
                ...
                1 ,
                1 ,
                weightInKgs = oneRepMax.weightKgs
            ) ,
            schema
        )
    }
